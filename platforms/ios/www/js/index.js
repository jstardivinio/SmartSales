...
var user = {
    ...
    User: function (uname, phone, email) {
...
/*
        ...
        tx.executeSql("SELECT * FROM USERS WHERE UName=? AND Password=? ", [uname, password], function (tx, results) {
           ...
           /*
            if (len > 0) {
                ...
                setUser(results.rows.item(0).UName, results.rows.item(0).Phone, results.rows.item(0).Email);
    ...
    setUser("Cjordan", 678289617, "jstar.divinio@gmail.com");
    navigator.notification.alert(
        'Welcome Chinda',  // message
        '',         // callback
        'Welcome',            // title
        'Thanks'                  // buttonName
    );
    setView("mainDiv", views['home']);
    //setView("mainDiv", views['paymentMethod']);

};

//set the current users data
function setUser(name, phone, email) {
    app.user = new user.User(name, phone, email);
};
...
function displayServicesByCathegory(svc) {
    ...
    for (i = 1; i <= len; i++) {
        ...
        if (svc[k]["category"] == "Electricity & Water Bills") {
        ...
        } else if (svc[k]["category"] == "Mobile & Phone - Prepaid Topup") {
        ...
        } else if (svc[k]["category"] == "Cable & Pay TV") {
        ...
        } else if (svc[k]["category"] == "Online Topup & Internet Services") {
        ...
        } else if (svc[k]["category"] == "Insurance & Investment") {
...
function setServiceView1(id) {
    if (app.serviceArray[id]["type"] == "SEARCHABLE_BILL" || app.serviceArray[id]["type"] == "NON_SEARCHABLE_BILL") {
        app.dinamicSId = id;
...
function setPaymentView() {
    ...
    var v =
        ...
        '<img class="img-fluid" alt="Responsive image" src="../img/cash1-en.jpg">' +
        '</label>' +
        '</div>' +
        ...
        '<img class="img-fluid" alt="Responsive image" src="../img/mtn.jpg">' +
        '</label>' +
        '</div>' +
        ...
        '<img class="img-fluid" alt="Responsive image" src="../img/orange.jpg">' +
        '</label>' +
        '</div>' +
        ...
        '<img class="img-fluid" alt="Responsive image" src="../img/express_union.png">' +
        '</label>' +
        '</div>' +
...
function processPayment() {
    ...
    var custoOtp = optID == "orange" ? $("#opt_one_time_password").val : null;
    alert(optID + custoPhone + custoName + custoOtp);
    var today = new Date();
        ...
        if (resp == "submited") {
            ...
            for (i = 0; i <= limit; i++) {
                ...
                if (status == "CONFIRMED" || status == "FAILED") {
                    ...
                    if (i == limit) {
                        alert("Confirmation time out");
                        setView("submitPayment", "Submit");
                        break;
                    }
                    alert("Check payment status");
                    sleep(10000);

                    path = "/s3papi/orderstatus";
                    queryParam = "?txnid=" + q['qId'];
...
//check the enkap order status
/*function checkEnkapOrderStatus(id){
    path = "/s3papi/orderstatus";
    queryParam = "?txnid=" + id;
    //var stat;
    ...
    cordova.plugin.http.get(
        ...
        function (response) {
            // prints 200
            //alert(response.status);
            stat = JSON.parse(response.data);
            app.dinamicEnkapStatus = stat;
            //stat = JSON.parse(response.data);
        ...
        function (response) {
            ...
            //prints Permission denied
            alert(response.error);
            setView("processPayment", "Make Payment");
            return false;
        });
